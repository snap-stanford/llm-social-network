...
def parse_gpt_output(G, output, ego_friend, p, prompt='singles'):
    pairs = []
    if ('\n' in output):
        lines = output.split('\n')
    else:
...
if __name__ == "__main__":
    personas = load_personas_as_dict('programmatic_personas.txt')
    i = 30
    while (i < 35):
        G = generate_network(personas, format='no_names', perspective='second', rand='on')
